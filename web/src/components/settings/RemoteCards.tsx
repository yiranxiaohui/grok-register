...
export function RemoteCards() {
  ...
  const enforceExclusive = useCallback(
    (source: "grok2api" | "cpa" | "load", next?: { gp?: boolean; gr?: boolean; cp?: boolean; cr?: boolean }) => {
      const gp = next?.gp ?? gProbe;
      const gr = next?.gr ?? gRelogin;
      const cp = next?.cp ?? cProbe;
      const cr = next?.cr ?? cRelogin;
      const gOn = gp || gr;
      const cOn = cp || cr;
      if (source === "grok2api" && gOn) {
        setCProbe(false);
        setCRelogin(false);
        setBackend("grok2api");
        return;
    ...
    [gProbe, gRelogin, cProbe, cRelogin],
  );
  ...
  const loadBackend = useCallback(async () => {
    try {
      ...
      setBackend(b);
      setBackendStatus({ text: "当前：" + (b ? (b === "cpa" ? "CPA" : "Grok2API") : "未锁定"), kind: b ? "ok" : "warn", show: true });
    } catch {
  ...
  useEffect(() => {
    ...
    loadCpa().catch(() => {});
    loadBackend().catch(() => {});
  }, [loadGrok2api, loadCpa, loadBackend]);
  ...
  const run = (fn: () => Promise<void>, which: "g" | "c" | "b") => {
    fn().catch((err) => {
      const setter = which === "g" ? setGStatus : which === "c" ? setCStatus : setBackendStatus;
  ...
  const saveBackend = async () => {
    ...
    setBackend(b);
    setBackendStatus({ text: "当前：" + (b ? (b === "cpa" ? "CPA" : "Grok2API") : "未锁定"), kind: b ? "ok" : "warn", show: true });
    toast(data.message || "远端对接：" + (b ? (b === "cpa" ? "CPA" : "Grok2API") : "未锁定"));
      ...
      <div className="card-block">
        <div className="section-head" style={{ padding: 0, border: 0, marginBottom: 10 }}>
          <div>
            ...
            <p className="muted" style={{ margin: "4px 0 0", fontSize: 12 }}>Grok2API 与 CPA 二选一。自动导入 / 拉取远端状态都只走当前锁定的一边；另一边即使勾选也不会执行。</p>
          </div>
